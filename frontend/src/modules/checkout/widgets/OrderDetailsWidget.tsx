...
import {stepValueFromQuoteResponse} from "@/modules/checkout/stepParams";
...
export function OrderDetailsWidget({
    ...
    const [order, setOrder] = useState<Quote["order"]>(() => value?.order ?? []);
    const [isSaving, setIsSaving] = useState(false);
    const [saveError, setSaveError] = useState<string | null>(null);

    const dirtyRef = useRef(false);
    const pendingOrderRef = useRef<Quote["order"]>(value?.order ?? []);
    const saveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
    const inFlightRef = useRef(false);
    const queuedFlushRef = useRef(false);

    useEffect(() => {
        if (!value?.order || dirtyRef.current) {
            return;
        }
        setOrder(value.order);
        pendingOrderRef.current = value.order;
    }, [value]);
    ...
    const flushSave = async () => {
        ...
        try {
            ...
            if (pendingUnchanged) {
                ...
                const stepValue = stepValueFromQuoteResponse("OrderDetailsWidget", updated);
                if (stepValue) {
                    const next = stepValue as OrderDetailsStepValue;
                    onSubmit(next);
                    setOrder(next.order);
                    pendingOrderRef.current = next.order;
                }
            } else {
                queuedFlushRef.current = true;
            }
